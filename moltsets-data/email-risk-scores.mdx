...
Tools that return more than one address carry a companion field per address — `business_email_risk_score`, `personal_email_risk_score`, and a `personal_emails_risk_score` array index-aligned with `personal_emails`. `reverse_email_lookup` keeps its own field names. `search_people` rows carry `business_email_risk_score` alongside each contact.
