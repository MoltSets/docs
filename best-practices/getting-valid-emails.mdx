...
<Note>
  \* The two personal-only endpoints are **not available on every plan**. Access is set per account, not by price tier — call `get_account` and read `personal_email_available` to know where yours stands. Where it's `false`, use `linkedin_to_business_email` instead, or contact support@moltsets.com to request access.
...
MoltSets revalidates on a **90-day** freshness window — a cached validation older than that is re-checked on the next lookup, across every email endpoint. There is no shorter window on `linkedin_to_best_personal_email` or anywhere else.
