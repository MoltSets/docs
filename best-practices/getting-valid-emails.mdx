...
<Note>
  \* The two personal-only endpoints are **not available on the \$997 plan**. On that plan, use `linkedin_to_business_email` instead.
...
**Personal** (`linkedin_to_personal_email`, `linkedin_to_best_personal_email` — not available on the \$997 plan) suits recruitment, consumer marketing, and re-engagement. It's also the only address that survives a job change, which makes it the better choice for contacts who may have left the employer you have on file.
