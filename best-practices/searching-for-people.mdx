...
<Note>
  Tokens are only charged when data is found and returned. A search that matches nothing costs nothing — but it still counts against your rate limit. A zero-hit search is HTTP `200` with `status: "not_found"` and `total: 0`, not an error. See [Rate Limits](/getting-started/rate-limits).
</Note>
...
**Drop `query` entirely when filters can carry the whole intent.** "VPs of sales at US software companies" is fully expressible as `seniority`, `department`, `industry`, and `country` — as a pure filter search it runs much faster than the same request phrased as free text.
...
`country`, `state`, and `city` all filter the **person's** location.

- `country` and `city` require exact stored values. Use full English names, not abbreviations.
- `state` accepts the full name or its abbreviation — `"Texas"` and `"TX"` both work.
...
```json
{ "title": ["CEO", "CMO"], "country": ["United States", "Canada"] }
```

That reads as (CEO or CMO) in (United States or Canada). It's one search instead of one per value, and every match is ranked together — so the first page holds the strongest results across all four combinations.

Lists work on `title`, `company`, `company_domain`, `country`, `state`, `city`, `seniority`, `industry`, `department`, `functional_area`, `employee_range`, `revenue_range`, `naics_code`, and `linkedin_industry`. Each value in a list follows the same matching rules as a single value — exact strings stay exact, and each `title` in the list is analysed on its own.

```json
{ "company_domain": ["acme.com", "stripe.com", "globex.com"], "seniority": ["VP", "C Suite"] }
```
...
Every response carries a top-level `total`, the count of matching records across all pages (`results.total` in the raw HTTP body). Read it **before** paginating to decide whether the search is worth walking, then increment `offset` by your `limit`. The `limit: 1` probe above gets you that total for the price of a single record.
