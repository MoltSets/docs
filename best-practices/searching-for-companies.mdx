...
One record costs one search record against your fair use pool. The same query at `limit: 25`, walked five pages deep, costs 125 — and every filter here is exact-match, so a single wrong string spends all 125 on the wrong list. The usual culprits are all silent: a value that isn't in the enumeration at all, a `linkedin_industry` label capitalised differently from the stored one, or a `country` filter pulling a multinational to the wrong office.
...
| Filters alone, no `query` | **Much faster and more precise** |
...
**Drop `query` entirely when filters can carry the whole intent.** Most account lists are pure firmographics — "software companies with 51–200 employees and $10M–$20M revenue" needs no free text at all, and runs much faster without it.
...
<Note>
  This is about speed, not billing. Each record returned counts as one search record against your plan's limits under the [Fair Use Policy for Search](https://moltsets.com/fair-use-policy-search/), whatever the query shape — a filter-only query returns faster and puts less load on search capacity, but it's billed the same.
</Note>
...
## Match several values in one search

Every positive filter accepts a single value or a list. A list is **OR within its field** and **AND across fields**:

```json
{ "industry": ["Information Technology", "Finance and Banking"], "country": ["United States", "Canada"] }
```

That reads as (Information Technology or Finance and Banking) in (United States or Canada). It's one search instead of one per value, and every match is ranked together.

Lists work on `domain`, `industry`, `employee_range`, `revenue_range`, `naics_code`, `linkedin_industry`, `country`, and `state`. Each value in a list follows the same exact-match rules as a single value. Adjacent bands are the common case:

```json
{ "employee_range": ["51-200", "201-500"], "revenue_range": ["$10M - $20M", "$20M - $50M"] }
```

<Note>
  A list widens the search, so `total` grows with it. Probe at `limit: 1` after adding values, the same as for any new search.
</Note>
...
`country` requires the exact stored value with standard capitalisation. `state` accepts the full name or its abbreviation — `"Texas"` and `"TX"` both work. Combine `state` with `country` for precision.
...
If you don't need a set number of contacts per account, skip the loop. `company_domain` accepts a list, so one call searches every account at once and ranks the matches together:

```javascript
const people = await search("search_people", {
  company_domain: companies.results.results.map((c) => c.domain),
  seniority: "VP",
  limit: 25
});
```

The trade-off is coverage: one ranked pool can return several people from a strong account and none from a weaker one. Keep the loop when every account needs contacts.
...
Read the top-level `total` before walking pages (`results.total` in the raw HTTP body), then increment `offset` by your `limit`. Every page you walk draws from the same search-record pool, so confirm the search is right at `limit: 1` before you commit to walking it.
