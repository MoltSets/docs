...
One record costs one search record against your fair use pool. The same query at `limit: 25`, walked five pages deep, costs 125 — and every filter here is exact-match, so a single wrong string spends all 125 on the wrong list. The usual culprits are all silent: `"$1 - $1M"` instead of `"$500k - $1M"`, `"TX"` instead of `"Texas"`, `"Information Technology"` where the records are filed under `"Computer Software"`, or a `country` filter pulling a multinational to the wrong office.
