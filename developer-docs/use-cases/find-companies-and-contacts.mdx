...
If you already have a LinkedIn URL, email, or IP address and want to enrich it, see [I Have X, I Want Y](developer-docs/use-cases/i-have-x-i-want-y).

<Warning>
  Search results are capped at **25 per call**. Use `limit` and `offset` together to paginate through larger result sets. See [Pagination](#pagination) below for examples.

  The following filters are not available on either search endpoint: location, founding year, and free-text industry or size. Industry and employee range use fixed enumerations — see the API reference for accepted values.
</Warning>
...
| Contacts matching a full ICP profile | Find contacts at information technology companies with 51–200 employees and revenue between $10M and $50M | `industry`, `employee_range`, `revenue_range` |
