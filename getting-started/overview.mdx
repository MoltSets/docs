...
<Note>
  `linkedin_to_personal_email` and `linkedin_to_best_personal_email` are **not available on the \$997 plan**.
...
<Note>
  \* `linkedin_to_personal_email` and `linkedin_to_best_personal_email` are **not available on the \$997 plan** — see [Plans](#plans) for the error they return. On that plan, use `linkedin_to_business_email` instead.
</Note>
