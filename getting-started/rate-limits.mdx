...
Rate limits vary by plan. All limits are per account — every API key and connector on the account draws from the same pool, so adding keys doesn't add headroom.
...
| \$397 / month | 150k | 750k | 750k |
| \$997 / month | 225k | 1.125M | 1.125M |
...
| \$397 / month | 75k | 375k | 375k |
| \$997 / month | 112.5k | 562.5k | 562.5k |

## Phone tokens

`linkedin_to_mobile_phone` spends **phone tokens** — a separate allowance from your regular token balance and from the Fair Use windows above. One phone token is spent per returned result; a miss costs nothing.
...
When the allowance runs out, calls return `402` with `insufficient_phone_tokens`; a batch drops the items it can't cover and returns the rest. Each call reports `phone_tokens_used` and `phone_tokens_remaining` in `metadata`.
...
Every API call counts toward the rate limit, regardless of whether it returns data — a `not_found` is still a request. Each item of a batch counts separately, so a batch of 100 counts as 100 requests. The account tools (`get_account`, `get_billing`, `get_usage`) don't count toward the enrichment or search Fair Use limits.
...
**Batch endpoints** where available (e.g. `linkedin_to_mobile_phone` supports up to 100 `linkedin_urls` per call) cut the number of HTTP calls you make — but not your request count. Each item in a batch still counts as one request, so a batch of 100 draws 100 requests from your Fair Use requests limit.