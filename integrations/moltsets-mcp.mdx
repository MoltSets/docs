...
<Tabs>
  ...
  <Tab title="Claude Desktop">
    Claude Desktop shares the same connectors as your Claude account — no config file editing required, and a connector you add on Claude.ai is already available here.

    Add MoltSets using the steps for your account type:

    - **Personal account (Free, Pro, or Max):** in Desktop, open **Customize → Connectors → + → Add custom connector**, paste `https://mcp.moltsets.com/mcp` into **Remote MCP server URL**, then add your **OAuth Client ID** and **OAuth Client Secret** under **Advanced settings**.
    - **Team or Enterprise account:** an Owner adds the connector once in **Organization settings → Connectors** (Add → Custom → Web); each member then opens **Customize → Connectors** and clicks **Connect** on the MoltSets connector. See the **Claude (claude.ai)** tab for the full owner and member steps — the connector carries over to Desktop.

    After adding or connecting, **restart Claude Desktop** so it picks up the connector.
